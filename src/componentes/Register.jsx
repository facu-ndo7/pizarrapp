...
function Register({ change, onRegister }) {
  let userName = useRef();
      ...
      fetch("http://localhost:8080/API/registerUsers.php", {
        ...
        .then(respuesta => {
          if (!respuesta.ok) {
            ...
            localStorage.setItem("estado", "registrado");
            onRegister(); //actualiza el estado en App
          }
        })
        .catch(error => {
  ...
  return (
    <div className="loginRegister-container">
      ...
      <button className="loginRegister-button" onClick={() => registerUser(userName, userEmail, userPassword)}>
        Register
      </button>
      <p className="loginRegister-question" onClick={() => change()}>
        Have you an account? Click Here
      </p>
    </div>
  );
}
